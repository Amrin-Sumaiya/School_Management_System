...
const Add_Exam = () => {
  ...
  const submitForm = async(e) => {
    e.preventDefault();
    await axios.post("http://localhost:8000/api/exam/exams", examData)
    .then((response)=>{
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-orange-50 rounded-xl shadow-lg mt-10">
      ...
      <h2 className="text-2xl font-semibold text-center text-black mb-6">Add Exam Information</h2>
      <form onSubmit={submitForm} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-sans text-black">Exam Date</label>
          <input
            type="date"
            name="examDate"
